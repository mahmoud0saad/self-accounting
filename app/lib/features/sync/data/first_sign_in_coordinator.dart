...
Future<int> runFirstSignInAccountSync(
  ...
  Future<void> Function()? onBeforeRestorePrompt,
}) async {
  ...
  try {
    ...
    final catalogTotals =
        catalogSnapshot['totals'] as Map<String, dynamic>? ?? {};
    ...
    if (catalogHas || challengeHas) {
      await onBeforeRestorePrompt?.call();
    ...
    await sync.drainOutbound();
    await sync.pullDeltas();
